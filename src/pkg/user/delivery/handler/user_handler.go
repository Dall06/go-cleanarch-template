...
import (
	...
	"github.com/Dall06/go-cleanarch-template/src/pkg/user/delivery"
	"github.com/Dall06/go-cleanarch-template/src/pkg/user/usecase"
)

type UserHandler struct {
	UserInteractor *usecase.UserInteractor
	ResponseHandler	*server.ResponseHandler
	JwtHandler	*server.JWTHandler
	LoggerHandler *server.LoggerHandler
...
func NewUserHandler(ai *usecase.UserInteractor) *UserHandler {
	return &UserHandler{
		...
		JwtHandler: server.NewJWTHandler(),
		LoggerHandler: server.NewLoggerHandler(),
